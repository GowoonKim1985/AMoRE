...
int CupGeneralTCB::Open()
{
  ...
  int state = fTCB->Open();
  if (state != 0) {
    fLog->Error("CupGeneralTCB::Open", "tcb open failed, check power or link status");
  } else {
...
bool CupGeneralTCB::Config()
{
  ...
  for (int i = 0; i < nconf; i++) {
    ...
    if (name.Contains("TCB"))
      continue;
    if (!conf->IsEnabled() || !conf->IsLinked())
      continue;
    ...
    if (dramon) {
      fLog->Info("CupGeneralTCB::Config", "%s[mid=%2d] DRAM on", name.Data(), mid);
    } else {
      fLog->Error("CupGeneralTCB::Config", "%s[mid=%2d] error occurred during turning DRAM on", name.Data(), mid);
      return false;
    }
  ...
  if (fTCBType == TCB::AMORE1) {
    ...
    if (dramon) {
      fLog->Info("CupGeneralTCB::Config", "TCB[mid=0] DRAM on");
    } else {
      fLog->Error("CupGeneralTCB::Config", "TCB[mid=0] error occurred during turning DRAM on");
      return false;
    }
    fTCB->AlignDRAM(0);
  }
  ...
  for (int i = 0; i < nconf; i++) {
    ...
    if (name.EqualTo("TCB")) {
      ...
      if (!name.EqualTo("STRG")) {
        fLog->Warning("CupGeneralTCB::Config", "unknown kind of module : %s", name.Data());
      }
      continue;
    }
  }
...
bool CupGeneralTCB::ConfigFADC(FADCTConf* conf)
{
  ...
  if (!conf->IsEnabled()) {
    fLog->Info("CupGeneralTCB::ConfigFADC", "%s[mid=%2d] is disabled, passed", conf->GetName(), mid);
    return true;
  }

  if (!conf->IsLinked()) {
    fLog->Error("CupGeneralTCB::ConfigFADC", "%s[mid=%2d] is enabled but not linked", conf->GetName(), mid);
    return false;
  }
  ...
  fLog->Info("CupGeneralTCB::ConfigFADC", "%s[mid=%2d] was configured", conf->GetName(), mid);
  return true;
}
...
bool CupGeneralTCB::ConfigGADC(GADCTConf* conf)
{
  ...
  if (!conf->IsEnabled()) {
    fLog->Info("CupGeneralTCB::ConfigGADC", "%s[mid=%2d] is disabled, passed", conf->GetName(), mid);
    return true;
  }

  if (!conf->IsLinked()) {
    fLog->Error("CupGeneralTCB::ConfigGADC", "%s[mid=%2d] is enabled but not linked", conf->GetName(), mid);
    return false;
  }
  ...
  fLog->Info("CupGeneralTCB::ConfigGADC", "%s[mid=%2d] was configured", conf->GetName(), mid);
  return true;
}
...
bool CupGeneralTCB::ConfigSADC(SADCTConf* conf)
{
  ...
  if (!conf->IsEnabled()) {
    fLog->Info("CupGeneralTCB::ConfigSADC", "%s[mid=%2d] is disabled, passed", conf->GetName(), mid);
    return true;
  }

  if (!conf->IsLinked()) {
    fLog->Error("CupGeneralTCB::ConfigSADC", "%s[mid=%2d] is enabled but not linked", conf->GetName(), mid);
    return false;
  }
  ...
  fLog->Info("CupGeneralTCB::ConfigSADC", "%s[mid=%2d] was configured", conf->GetName(), mid);
  return true;
}
...
bool CupGeneralTCB::ConfigIADC(IADCTConf* conf)
{
  ...
  if (!conf->IsEnabled()) {
    fLog->Info("CupGeneralTCB::ConfigIADC", "%s[mid=%2d] is disabled, passed", conf->GetName(), mid);
    return true;
  }

  if (!conf->IsLinked()) {
    fLog->Error("CupGeneralTCB::ConfigIADC", "%s[mid=%2d] is enabled but not linked", conf->GetName(), mid);
    return false;
  }
  ...
  fLog->Info("CupGeneralTCB::ConfigIADC", "%s[mid=%2d] was configured", conf->GetName(), mid);
  return true;
}
...
bool CupGeneralTCB::ConfigAmoreADC(AmoreADCConf* conf)
{
  ...
  if (!conf->IsEnabled()) {
    fLog->Info("CupGeneralTCB::ConfigAmoreADC", "%s[mid=%2d] is disabled, passed", conf->GetName(), mid);
    return true;
  }

  if (!conf->IsLinked()) {
    fLog->Error("CupGeneralTCB::ConfigAmoreADC", "%s[mid=%2d] is enabled but not linked", conf->GetName(), mid);
    return false;
  }
  ...
  fLog->Info("CupGeneralTCB::ConfigAmoreADC", "%s[mid=%2d] was configured", conf->GetName(), mid);
...
bool CupGeneralTCB::ConfigTCB(TCBConf* conf)
{
  ...
  fLog->Info("CupGeneralTCB::ConfigTCB", "TCB[sid=%d] was configured", conf->SID());
...
int CupGeneralTCB::CheckLinkStatus()
{
  ...
  for (int i = 0; i < nport; i++) {
    if (linked[i]) {
      ...
      if (fADCType == ADC::NONE)
        conf = fConfigs->FindConfig(linkedMID[i]);
      else
      ...
      if (conf) {
        conf->SetLink();
        if (conf->IsEnabled()) {
          fLog->Info("CupGeneralTCB::CheckLinkStatus", "%s[mid=%2d] found @ %d", conf->GetName(), linkedMID[i], i + 1);
          nlinked += 1;
        } else {
          fLog->Warning("CupGeneralTCB::CheckLinkStatus", "%s[mid=%2d] found @ %d, but disabled", conf->GetName(),
              linkedMID[i], i + 1);
        }
      }
    }
      ...
      if (linked[i]) {
        if (linkedMID[i] <= 40) {
          ...
          if (conf) {
            ...
            if (conf->IsEnabled()) {
              fLog->Info("CupGeneralTCB::CheckLinkStatus", "FADCT[mid=%2d] found @ %d", linkedMID[i], i + 1);
              nlinked += 1;
            } else {
              fLog->Warning("CupGeneralTCB::CheckLinkStatus", "FADCT[mid=%2d] found @ %d, but disabled",
                  linkedMID[i], i + 1);
            }
          } else {
            ...
            if (conf) {
              ...
              if (conf->IsEnabled()) {
                fLog->Info("CupGeneralTCB::CheckLinkStatus", "GADCT[mid=%2d] found @ %d", linkedMID[i], i + 1);
                nlinked += 1;
              } else {
                fLog->Warning("CupGeneralTCB::CheckLinkStatus", "GADCT[mid=%2d] found @ %d, but disabled",
                    linkedMID[i], i + 1);
              }
            }
          }
          ...
          if (conf->IsEnabled()) {
            fLog->Info("CupGeneralTCB::CheckLinkStatus", "SADCT[mid=%2d] found @ %d", linkedMID[i], i + 1);
            nlinked += 1;
          } else {
            fLog->Warning("CupGeneralTCB::CheckLinkStatus", "SADCT[mid=%2d] found @ %d, but disabled",
                linkedMID[i], i + 1);
          }
        }
        if (linkedMID[i] > 128) {
          ...
          if (conf->IsEnabled()) {
            fLog->Info("CupGeneralTCB::CheckLinkStatus", "AMOREADC[mid=%2d] found @ %d", linkedMID[i], i + 1);
            nlinked += 1;
          } else {
            fLog->Warning("CupGeneralTCB::CheckLinkStatus", "AMOREADC[mid=%2d] found @ %d, but disabled",
                linkedMID[i], i + 1);
          }
        }
      }

...
void ratio1_wcmd_1wk_all()
{
  ...
  const int fn = 9;
  int run[fn] = {304, 311, 315, 318, 327, 330, 339, 356, 368};
  int finsub[fn] = {791, 190, 191, 689, 282, 695, 286, 667, 260};
  ...
  for(int proc=0; proc<procnum; proc++){
    ...
    TString outfile = Form("./ratio1_1wk/run%i_%04d_to_%04d.root",run[lp], sub1, sub2);
  ...
  TFile * plotfile = new TFile(Form("./plot/run%i_div%i_1wk.root",run[lp],subdiv),"RECREATE");
  hmean->Write();
  plotfile->Close();

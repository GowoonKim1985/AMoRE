...
void qfit_wcd_genrun(int run = 435)
{

  //  run=437;
  //  run =434;
  int isub=0;
  int fsub=isub+(7*24*2)-1;
  int mb, mb_sp;
 ...
 char a;
  ...
  f = new TFile(Form("./hist/QmaxDist_%06d.%05d_%05d.root", run, isub, fsub));      
  ...
  for(int j = 0; j < 47; j++){
	...
	np_sp[j]++;
      ...
      */
      ...
      ffunc[j] -> SetParLimits(3, 0, max_sp+30);
      ...
      ffunc[j] -> SetParameter(5, 10);      
	    ...
	    if(j==2){ffunc[j] -> SetParLimits(4, 30, 80);};//pmt3	    
	    if(j==2){ffunc[j] -> SetParameter(5, 2);};//pmt3
	    ...
	    if(run==435){
	      if(j==2){ffunc[j] -> SetParameter(5, 15);};//
	      if(j==4){ffunc[j] -> SetParameter(5, 15);};//
	      if(j==6){ffunc[j] -> SetParameter(5, 15);};//	      	      
	    }	    
...
std::cout << "Reduced Chi2: " << fitResult->Chi2() / fitResult->Ndf() << std::endl;
 ...
 chi2=(fitResult->Chi2()) / (fitResult->Ndf());
  ...
  TFile of(Form("./hist/GainCal_%06d.%05d_%05d.root", run, isub, fsub), "recreate");

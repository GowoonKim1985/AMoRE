...
void qmax_wcd_genrun(int run=435)
{
  ...
  for(int i=isub; i<=fsub; i++){
  t->Add(Form("/data/amore2test/RAW/%06d/FADC_%06d.root.%05d", run, run, i));    
  }
  ...
  cout << nevt << endl;
    ...
    //    cout<<"entry " <<i <<" trgtype " <<trgtype<<endl;
  ...
  TFile of(Form("./hist/QmaxDist_%06d.%05d_%05d.root", run, isub, fsub), "recreate");  
  for(int i = 0; i < 47; i++)
    his[i] -> Write();
  of.Close();

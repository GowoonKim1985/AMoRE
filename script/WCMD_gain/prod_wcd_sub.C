...
R__LOAD_LIBRARY(/home/kkw/muon_3.1.2/shlib/Linux5.14-GCC_11_5/libMuonObjs.so)
//R__LOAD_LIBRARY(/home/kkw/muon_3.1.2/shlib/Linux5.14-GCC_11_5/libRawObjs.so)

#include "/home/kkw/muon_3.1.2/MuonObjs/MuonObjs/WCEvent.hh"
...
//  TFile *of = new TFile(Form("/home/kkw/muon_3.1.2/test/data/WCMD/%06d/prd_wcd_%06d_%05d.root", runnum, runnum, subrun), "recreate");
  TFile *of = new TFile(Form("./data/prd_wcd_all_%06d_%05d.root", runnum, subrun), "recreate");
  TTree *otree = new TTree("prd_wcd", "prd_wcd");
  //  otree->Branch("fTriggerType", &trgtype, "fTriggerType/s");
...
//  unsigned short ped;
  ...
  for (int i = 0; i < 10; i++) {
   ...
   if(info -> GetTriggerType() == 1) continue;
    ...
    //    wevent -> SetTriggerType(info -> GetTriggerType());    

    
    trgtype = info->GetTriggerType();
    cout<<"i : trgtype "<<i<<" : "<<trgtype<<endl;
    int nch = data -> GetN();
    for(int j = 0; j < nch; j++){
	...
	if(adcp > dum){
      ...
      //std::cout << "Before Fill: WCEvent Address = " << wevent << std::endl;
    otree -> Fill();
    //    std::cout << "After Fill: WCEvent Address = " << wevent << std::endl;
  }
}
  // otree->BranchRef();
  otree -> Write();

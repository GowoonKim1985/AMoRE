void plot_wcd()
{
  ...
  //  chain -> Add(Form("/home/kkw/PROD/WCMD/%06d/prd_wcd_%06d_%05d.root",run,run,subrun));
  chain -> Add(Form("/home/kkw/PROD/WCMD/%06d/prd_wcd_%06d_*.root",run,run));      
  ...
  cout << hqsum_sel -> Integral(45, 500)/(600*600+300*600*4)/337706.20 << endl;
  ...
  //  TFile *of = new TFile(Form("/home/kkw/muon_3.1.2/script/hist/hist_wcd_%06d_%05d.root",run,run,subrun),"recreate");
  TFile *of = new TFile(Form("/home/kkw/muon_3.1.2/script/hist/hist_wcd_%06d.root",run),"recreate");    
  hqsum->Write();
  hqsum_sel->Write();
  for(int ihis=0;ihis<48;ihis++){
    hqtot[ihis]->Write();
    hqtot_sel[ihis]->Write();
  }
    htbit->Write();
    htbQ->Write();
    hhitmap->Write();

...
void trg_calculation()
{
  ...
  for(int sub=0; sub<=finsub[ifile]; sub++){
  //  for(int sub=0; sub<=0; sub++){
    chain -> Add(Form("/home/PROD/WCMD/%06d/prd_wcd_%06d_%05d.root",run[ifile],run[ifile],sub));
  }
  ...
  TString outfile = Form("./div/run%i_rate_10s.root",run[ifile]); 
    of[ifile] = new TFile(outfile.Data(), "RECREATE");

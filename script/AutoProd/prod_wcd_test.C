...
//  TFile *of = new TFile(Form("/home/kkw/PROD/WCMD/%06d/prd_wcd_%06d_%05d.root", runnum, runnum, subrun), "recreate");
...
//  unsigned short ped;
  ...
  for (int i = 0; i < 100; i++) {
    ...
    //            hit -> SetVariables(tbit[id[j]],qtot[id[j]], qmax[id[j]], fmax[id[j]], fmaxx[id[j]], ped, rms);

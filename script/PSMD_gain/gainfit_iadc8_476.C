...
void gainfit_iadc8_476()
{
  ...
  TFile * out = new TFile(Form("./gain/gain_%06d_iadc8.root",run), "recreate");
  ...
  for(int i = idet; i <= fdet; i++) {//48-58, 113-123
    for(int j=0; j<4; j++){
  ...
  for(int i = 0; i < tdet; i++) {
	...
	//	lower[j] = maxx[j] - std[j];
		...
		lower[j] = maxx[j] - 50;
				...
				cout<<lower[j]<<" "<<upper[j]<<endl;
		if(lower[j]<0){
		  qhis[i][j]->GetXaxis()->SetRangeUser(60,350);
		  maxbin[j] = qhis[i][j]->GetMaximumBin();
		  maxx[j] = qhis[i][j]->GetBinCenter(maxbin[j]);		  
		  upper[j] = maxx[j] + 50;
		  lower[j] = maxx[j] - 50;	
		...
		if((i==66&&j==0)||
		   ...
		   (i==69&&j==2)||
		   (i==69&&j==3)){
		  qhis[i][j]->GetXaxis()->SetRangeUser(100,300);
		  maxbin[j] = qhis[i][j]->GetMaximumBin();
		  maxx[j] = qhis[i][j]->GetBinCenter(maxbin[j]);

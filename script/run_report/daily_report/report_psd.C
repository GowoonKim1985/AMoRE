...
void report_psd()
{
  ...
  FILE *fin_file1;
  FILE *fin_file2;
  TString fin_filename1 = Form("/home/kkw/PROD/PSMD/%06d/prd_psd_daq1_%06d_%05d.root",run,run,subrun);
  TString fin_filename2 = Form("/home/kkw/PROD/PSMD/%06d/prd_psd_daq2_%06d_%05d.root",run,run,subrun);
  fin_file1 = fopen(fin_filename1, "r");
  fin_file2 = fopen(fin_filename2, "r");  

  if(fin_file1 ==NULL){
  fin_filename1 = Form("/home/kkw/PROD/PSMD/%06d/prd_psd_daq1_%06d_%05d.root",run,run,subrun-1);
  fin_filename2 = Form("/home/kkw/PROD/PSMD/%06d/prd_psd_daq2_%06d_%05d.root",run,run,subrun-1);  
  }
  //  cout<<run<<endl;
  //  cout<<subrun<<endl;
  ...
  */
  ...
  // 전체 캔버스로 돌아가 타이틀을 추가
hcan->cd(); // 캔버스로 돌아가기
hcan->SetTopMargin(0.05);  // 캔버스 상단 여백을 줄임

// TPaveText로 타이틀 추가
TPaveText *pt = new TPaveText(0.1, 0.92, 0.9, 0.98, "NDC");  // NDC 좌표에서 상단 중앙에 텍스트 박스 생성

...
void report_wcd()
{
  ...
  //recent file check
  FILE *fin_file;
  TString fin_filename = Form("/data/amore2test/PROD/WCMD/%06d/prd_wcd_%06d_%05d.root",run,run,subrun);

  fin_file = fopen(fin_filename, "r");
//  TString fin_filename = Form("/home/kkw/PROD/WCMD/%06d/prd_wcd_%06d_%05d.root",run,run,subrun);
  if(fin_file ==NULL){
  fin_filename = Form("/data/amore2test/PROD/WCMD/%06d/prd_wcd_%06d_%05d.root",run,run,subrun-1);
  }
  //  cout<<run<<endl;
  //  cout<<subrun<<endl;

...
void fit_psmd_eff()
{
  ...
  int cut_qsum = 10000;
  int cut_qsum_fully = 5000;

  int cut_qsum_ff = 0;
  int mean = 16000;

  double range[2]  = {5000, 17500};
  ...
  TF1 *fitfunc, *bexp, *mLand;
    ...
    TH1D * h = (TH1D*)f->Get("det_qsum_sel");
    h -> SetDirectory(0);
       h -> Rebin(2);
    h->GetXaxis()->SetRangeUser(3000, 35000);
    ...
    fitfunc -> SetParameter(4, 1000);
    fitfunc -> SetParLimits(4, 500, 3000);
...
cout << x_cross << endl;
 ...
 double x_landau_min = mLand->GetX(1.0, 0, fitfunc->GetParameter(3));  // MPV보다 왼쪽에서 탐색
cout << "Landau fit < 1 : " << x_landau_min << endl;
}

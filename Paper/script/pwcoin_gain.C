...
void pwcoin_pbot_gain(){
  ...
  psmd->Add("./prod_2nd/pwcoin_all_bot_only_cut.root");
  ...
  int ptent = psmd->GetEntries();
    ...
    psmd->GetEntry(i);
    evtnum = psmd->GetLeaf("pevtnum")->GetValue();
    trgtime = psmd->GetLeaf("ptrgtime")->GetValue();    
    bitsum = psmd->GetLeaf("pbitsum")->GetValue();   

    for(int ipbit=0; ipbit<bitsum; ipbit++){

      trgdet[ipbit] = psmd->GetLeaf("ptrgdet")->GetValue(ipbit);
      qsum[ipbit] = psmd->GetLeaf("pqsum")->GetValue(ipbit);
      qmax[ipbit] = psmd->GetLeaf("pqmax")->GetValue(ipbit);
  ...
  TString anafile1 = "./prod_2nd/cpwcoin_all_bot_only.root";

...
void draw_psmd_pbhis_bot(){
  ...
  for(int i = 0; i < 124; i++){
    ...
    Qp1hist[i] = new TH1D(Form("hqp1_%d", i), "", 300, 0, 60000);
    Qp2hist[i] = new TH1D(Form("hqp2_%d", i), "", 300, 0, 60000);
  ...
  */
  ...
  //test
    ...
    */
    ...
    of8.Close();
*/
    TFile of9("./hist/qhist_psmd_qpb.root", "recreate");
    for(int i=48; i<=58; i++){
      Qsumhist[i]->Write();
      Qp1hist[i]->Write();
      Qp2hist[i]->Write();      

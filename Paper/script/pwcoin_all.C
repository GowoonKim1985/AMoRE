...
void pwcoin_all()
{
  ...
  double pqsum[120], piq[120][4], wqtot[48], pqmax[120], pqmin[120],pqp1[120], pqp2[120];
  ...
  coin->Branch("wqmax_ch", &wqmax_ch, "wqmax_ch/I");  
  ...
  double dtime=1000;//200ns
	...
	if(abs(timedff)<=dtime){	
	  ...
	  for(int iwbit=0; iwbit<wbitsum; iwbit++){
	    ...
	    wqmax_ch = wcmd->GetLeaf("qmax_ch")->GetValue();
	    //   cout<<"qmax ch "<<wqmax_ch<<endl;
	  ...
	  for(int ipbit=0; ipbit<pbitsum; ipbit++){
	    ...
	    pqp1[ipbit]=piq[ipbit][0]+piq[ipbit][1];
	    pqp2[ipbit]=piq[ipbit][2]+piq[ipbit][3];	    
	  }
	  /*
  ...
  TString anafile = Form("./prod_2nd/pwcoin_%06d.root", runnum);

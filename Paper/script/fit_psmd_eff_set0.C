...
void fit_psmd_eff()
{
  ...
  int cut_qsum = 6000;
  int cut_qsum_fully = 3000;

  int cut_qsum_ff = 0;
  int mean = 9300;

  double range[2]  = {500, 13000};
  ...
  TF1 *fitfunc, *bexp, *mLand;
    ...
    TH1D * h = (TH1D*)f->Get("pan_qsum");
    h -> SetDirectory(0);
       h -> Rebin(2);
    h->GetXaxis()->SetRangeUser(2000, 20000);
    ...
    TF1 * expfit = new TF1("expfit", "expo", 2300,5000);
    h->Fit(expfit,"R");

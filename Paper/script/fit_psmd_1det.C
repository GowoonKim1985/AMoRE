...
cout << "mLand integral (0-17000) = "
     << mLand->Integral(10000, 17000) << endl;    
    
cout << "mLand integral (0-50000) = "
     << mLand->Integral(0, 10000) << endl;
...
//  double area = 1.67*0.3*22;//cm^2
  ...
  double divsum_err = sqrt(divsum1+divsum2);
    ...
    double mincut = mLand->GetX(1, 0, 50000);
    cout<<"cut : sum over 1 "<<mincut<<endl;
...
double y_max = h->GetMaximum(); // h1은 히스토그램 이름
...
// 1. 범례 객체 생성 (x1, y1, x2, y2) -> 우상단 위치 조정
// 0.6~0.88 사이의 값이 우상단에 적절합니다.
TLegend *leg = new TLegend(0.68, 0.73, 0.9, 0.9);

// 2. 범례 스타일 설정 (테두리 없애고 배경 투명하게)
leg->SetBorderSize(0);
leg->SetFillStyle(0);
leg->SetTextFont(42); // 폰트 굵게
leg->SetTextSize(0.03);

// 3. 각 요소 추가 ("l" 옵션은 Line 스타일로 표시한다는 의미)
leg->AddEntry(mLand, "Muon Candidate", "l"); // 파란 점선
leg->AddEntry(bexp, "Background", "l");       // 초록 점선
leg->AddEntry(line, "Cut Line", "l");        // 빨간 점선

...
OUT_DIR="/home/kkw/monitoring/rel"
...
# Find start time in current file
start_time=$(grep -m 1 -oP '\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}' "$FLOGFILE")

# Find end time in current file
end_time=$(tail -n 1 "$FLOGFILE" | grep -oP '\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}')
...
c_time=$((end_timestamp - start_timestamp))

#get total run time
RTFILE="/home/kkw/monitoring/rel/time.txt"
o_timeline=$(tail -n 1 "$RTFILE")
o_time=$(echo "$o_timeline" | grep -oE '[0-9]+$')

#echo "o time : $o_time"
#echo "s : $start_timestamp"
#echo "e : $end_timestamp"
#echo "dt : $time_difference"
...
echo -e "${c_time}">>${OUTFILE}
echo -e "${o_time}">>${OUTFILE}
cd $OUT_DIR

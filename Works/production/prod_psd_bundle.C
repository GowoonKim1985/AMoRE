...
#include "/home/kkw/Muon/muon_3.1.2/MuonObjs/MuonObjs/PSHit.hh"


int tbin;
double q, qa[10], pt, ttime;
...
void prod_psd_bundle(int daqtype = 2)
{
  int run = 330;
  ...
  for(int subrun=220; subrun<=696; subrun++){
    cout<<"subfile "<<subrun<<" process..."<<endl;
  ...
  chain->Add(Form("/home/kkw/Muon/RAW/%06d/IADC%d_%06d.root.%05d", run, daqtype, run, subrun));
  if(chain == NULL){
    cout << "!" << endl;
    return 0;
  }
...
//  TFile * out = new TFile(Form("/home/kkw/muon_3.1.2/test/data/PSMD/%06d/prd_psd_daq%d_%06d_%05d.root", run, daqtype, run, subrun), "recreate");
  TFile * out = new TFile(Form("/home/kkw/Muon/PROD/PSMD/%06d/prd_psd_daq%d_%06d_%05d.root", run, daqtype, run, subrun), "recreate");

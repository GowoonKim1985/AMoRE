...
#include "/home/kkw/Muon/muon_3.1.2/MuonObjs/MuonObjs/WCPmt.hh"

void prod_wcd_bundle()
{
  int runnum = 327;
  TStopwatch t;
  t.Start();

  for(int subrun = 0; subrun<=282; subrun++){
    cout<<"sub file "<<subrun<<" process..."<<endl;
  TChain * chain = new TChain("AbsEvent");
  chain -> Add(Form("/home/kkw/Muon/RAW/%06d/FADC_%06d.root.%05d", runnum, runnum, subrun));
  if(chain == NULL){
    cout << "!" << endl;
    return 0;
  }
  ...
  TFile *of = new TFile(Form("/home/kkw/Muon/PROD/WCMD/%06d/prd_wcd_%06d_%05d.root", runnum, runnum, subrun), "recreate");

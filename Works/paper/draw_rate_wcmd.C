...
void draw_rate_wcmd()
{
  ...
  int cnt1[n1],  ccnt1[n1];
  int cnt2[n2],  ccnt2[n2];
  double time_div1[n1], time_div2[n2];
  double iocc1[n1], cocc1[n1], iocc2[n2], cocc2[n2];
  
  for(int i = 0; i < n1; i++){
    time_div1[i] = 0;      cnt1[i] = 0; ccnt1[i] = 0; cocc1[i] = 0;
  }
  for(int i = 0; i < n2; i++){
    time_div2[i] = 0;
      cnt2[i] = 0; ccnt2[i] = 0; cocc2[i] = 0;
  }
  int hr_cnt;
  ...
  for(Long64_t i = 0; i < tent1; i++){
    ...
    timesec=trgtime1/1.e9;
      ...
      cnt1[hr_cnt-1]++;
      if((0.8 * qsum1 - 2100 > qmax1) && qsum1 > qcut){ ccnt1[hr_cnt-1]++;}
    ...
    if(timesec>(hr_cnt*3600*div)){
      ...
      time_div1[hr_cnt-1]=timesec/((double)div*3600); //1 day
    	iocc1[hr_cnt-1]=cnt1[hr_cnt-1];
	cocc1[hr_cnt-1]=ccnt1[hr_cnt-1];
      cout<<time_div1[hr_cnt-1]<< " day / occ : "<<iocc1[hr_cnt-1]<<"(w/ cut "<<cocc1[hr_cnt-1]<<")"<<endl;
  ...
  for(Long64_t i = 0; i < tent2; i++){
    ...
    timesec=trgtime2/1.e9;
      ...
      cnt2[hr_cnt-1]++;
      if((0.8 * qsum2 - 2100 > qmax2) && qsum2 > qcut){ ccnt2[hr_cnt-1]++;}
    
    if(timesec>(hr_cnt*3600*div)){

      time_div2[hr_cnt-1]=timesec/((double)div*3600); //1 day

	iocc2[hr_cnt-1]=cnt2[hr_cnt-1];
	cocc2[hr_cnt-1]=ccnt2[hr_cnt-1];
      cout<<time_div2[hr_cnt-1]<< " day / occ : "<<iocc2[hr_cnt-1]<<"(w/ cut "<<cocc2[hr_cnt-1]<<")"<<endl;

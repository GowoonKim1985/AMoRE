    ...
    */
    ...
    for(int j = 0; j < 5; j++){
      ...
      for(int k = 0; k < tbit1; k++){
        if(ch1[k] == pmtnum[j]){ ich=j; found = true; break; }
      }    
      if(found == true) {
        cnt1[hr_cnt-1][j]++;
	//        if((0.8 * qsum1 - 2100 > qmax1) && qsum1 > qcut){ ccnt1[hr_cnt-1][j]++; }
       if(qsum1[ich]>0){ ccnt1[hr_cnt-1][j]++; }
      }
    }
	...
	cocc1[hr_cnt-1][j]=ccnt1[hr_cnt-1][j];
      ...
      cout<<time_div1[hr_cnt-1]<< " hr : pmt no."<<pmtnum[3]<<" / occ : "<<iocc1[hr_cnt-1][3]<<"(w/ cut "<<cocc1[hr_cnt-1][3]<<")"<<endl;
...
*/
    ...
    for(int j = 0; j < 5; j++){
      ...
      for(int k = 0; k < tbit2; k++){
        if(ch2[k] == pmtnum[j]){ ich=j; found = true; break; }
      }    
      if(found == true) {
	      cnt2[hr_cnt-1][j]++;
	if(qsum2[ich]>0){ ccnt2[hr_cnt-1][j]++; }
	...
	cocc2[hr_cnt-1][j]=ccnt2[hr_cnt-1][j];
      ...
      cout<<time_div2[hr_cnt-1]<< " day : pmt no."<<pmtnum[3]<<" / occ : "<<iocc2[hr_cnt-1][3]<<"(w/ cut "<<cocc2[hr_cnt-1][3]<<")"<<endl;
...
int colors[5] = {kRed, kBlue, kGreen+2, kOrange+1, kViolet+1};
  ...
  //  g_cr[0]->GetYaxis()->SetRangeUser(0.0, 1.2*ymax);
    g_cr[0]->GetYaxis()->SetRangeUser(0.0, 100000.);

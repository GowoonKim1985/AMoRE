  ...
  */
  ...
  //test
    ...
    */
    ...
    TFile of1("./hist/qhist_psmd_2hit_grp1.root", "recreate");
    for(int i=0; i<=11; i++){
      Qsumhist[i]->Write();
    }
    of1.Close();

    TFile of2("./hist/qhist_psmd_2hit_grp2.root", "recreate");
    for(int i=12; i<=23; i++){
      Qsumhist[i]->Write();
    }
    of2.Close();

    TFile of3("./hist/qhist_psmd_2hit_grp3.root", "recreate");
    for(int i=24; i<=35; i++){
      Qsumhist[i]->Write();
    }
    of3.Close();
    
    TFile of4("./hist/qhist_psmd_2hit_grp4.root", "recreate");
    for(int i=36; i<=47; i++){
      Qsumhist[i]->Write();
    }
    of4.Close();

    TFile of5("./hist/qhist_psmd_2hit_grp5.root", "recreate");
    for(int i=65; i<=76; i++){
      Qsumhist[i]->Write();
    }
    of5.Close();
      
    TFile of6("./hist/qhist_psmd_2hit_grp6.root", "recreate");
    for(int i=77; i<=88; i++){
      Qsumhist[i]->Write();
    }
    of6.Close();

    TFile of7("./hist/qhist_psmd_2hit_grp7.root", "recreate");
    for(int i=89; i<=100; i++){
      Qsumhist[i]->Write();
    }
    of7.Close();

    TFile of8("./hist/qhist_psmd_2hit_grp8.root", "recreate");
    for(int i=101; i<=112; i++){
      Qsumhist[i]->Write();
    }
    of8.Close();

    TFile of9("./hist/qhist_psmd_2hit_grp9.root", "recreate");
    for(int i=48; i<=58; i++){
      Qsumhist[i]->Write();
    }
    of9.Close();

    TFile of10("./hist/qhist_psmd_2hit_grp10.root", "recreate");
    for(int i=113; i<=123; i++){
      Qsumhist[i]->Write();
    }
    of10.Close();
